...
''' method attempts to apply stronger promoter to gates in the circuit via iteration
    this method was left for individual gate score evaluation method also submitted in this folder'''
...
def engineer_circuit(rf,data,componentdict,prom_inc):
    x = 1.5
    xn = 1.05
...
def main():
    ...
    '''Apply stronger RBS to gates in the circuit'''

    if len(componentdict)==select_num:
        UCF_list = []
        for stronger_p in range(1,5):
            data = engineer_circuit(rf,data,componentdict,stronger_p)
    ...
    '''Wait for server to be ready with posted UCFs'''
    ...
    for UCF_name in UCF_list:
        ...
        '''download resultant logic circuit files to get scores'''
